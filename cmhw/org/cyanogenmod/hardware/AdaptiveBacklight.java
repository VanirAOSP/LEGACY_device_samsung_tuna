 ...
 *
 * Adapted for Tuna By Dan Trifan
 *
...
public class AdaptiveBacklight {
    ...
    private static String FILE_CAB = "/sys/class/backlight/s6e8aa0/acl_set";
    ...
    public static boolean isEnabled() {
        if (Integer.parseInt(FileUtils.readOneLine(FILE_CAB)) == 1) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean setEnabled(boolean status) {
        if (status == true) {
            return FileUtils.writeLine(FILE_CAB, "1");
        } else {
            return FileUtils.writeLine(FILE_CAB, "0");
        }
    }
} 
